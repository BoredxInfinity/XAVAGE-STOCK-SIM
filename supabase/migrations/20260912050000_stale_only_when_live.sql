-- =====================================================================
-- XAVAGE STOCK SIM :: let an out-of-hours order queue instead of bouncing
-- =====================================================================
-- place_order checked the quote's age before it worked out whether the book
-- was open, and rejected anything older than price_staleness_seconds (15
-- minutes) with "Price data is stale. Try again shortly."
--
-- That was survivable while the worker polled a shut exchange round the clock,
-- because quote_time never got old. Now that a closed market means idle -- no
-- feed requests, the price frozen where the session left it -- every quote is
-- "stale" fifteen minutes after the close, and the function was rejecting
-- every order placed outside market hours.
--
-- The function already knows what to do with those: its own closing line says
-- "Market is closed - order queued for the next session." It never reached it.
-- Queuing a limit order overnight to work at the open is ordinary, and this
-- silently removed it.
--
-- Staleness is a guard against filling at a price that has moved. It belongs
-- to a live book, so that is where it now applies. When the market is shut the
-- order queues, and matching -- which tests market_is_open on every pass --
-- fills it at the next session's prices, not at this frozen one.
-- =====================================================================
...
begin
  ...
  -- fills at the price that session prints. Checking it regardless rejected
  -- every out-of-hours order with "try again shortly", which was both wrong
  -- and impossible to act on: it would not have come right until Monday.
